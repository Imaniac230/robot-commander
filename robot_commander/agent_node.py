...
class AgentServer(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('speech_to_text_port', 8080)
        self.declare_parameter('language_model_port', 8081)
        self.declare_parameter('text_to_speech_port', 8082)
        self.declare_parameter('speech_to_text_model_file', '')
        self.declare_parameter('language_model_model_file', '')
        self.declare_parameter('text_to_speech_model_file', '')
        self.declare_parameter('language_model_initial_prompt_file', '')
        self.declare_parameter('language_model_initial_prompt_context', '')
        self.declare_parameter('language_model_grammar_file', '')
        # FIXME(tts-server): this is currently unused
        self.declare_parameter('text_to_speech_voice_type', 'announcer')
    ...
    def initialize(self):
        stt: str = self.get_parameter('speech_to_text_model_file').get_parameter_value().string_value
        if not Path(stt).is_file(): raise FileNotFoundError(f"Speech-to-text model file '{stt}' not found.")
        llm: str = self.get_parameter('language_model_model_file').get_parameter_value().string_value
        if not Path(llm).is_file(): raise FileNotFoundError(f"Language-model model file '{llm}' not found.")
        tts: str = self.get_parameter('text_to_speech_model_file').get_parameter_value().string_value
        ...
        llm_init_file: str = self.get_parameter('language_model_initial_prompt_file').get_parameter_value().string_value
        llm_context: str = self.get_parameter('language_model_initial_prompt_context').get_parameter_value().string_value
        llm_init: str = ""
        ...
        self.agent = Agent(
            WhisperCPP(STTParams(
                ...
                server_port=self.get_parameter('speech_to_text_port').get_parameter_value().integer_value
            )),
            LlamaCPP(LLMParams(
                model_path=llm,
                ...
                json_schema_file_path=grammar,
                server_hostname=hostname,
                server_port=self.get_parameter('language_model_port').get_parameter_value().integer_value,
                n_of_parallel_server_requests=1
            )),
            # TODO(tts-server): local tts server is only experimental for now
            BarkCPP(TTSParams(
                model_path=tts,
                voice=self.get_parameter('text_to_speech_voice_type').get_parameter_value().string_value,
                server_hostname=hostname,
                server_port=self.get_parameter('text_to_speech_port').get_parameter_value().integer_value
            )) if tts else None
        )
